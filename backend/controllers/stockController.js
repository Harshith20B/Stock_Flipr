...
const getAllStocks = async (req, res) => {
  try {
    // Fetch some popular stocks for sidebar
...
async function getStockDetails(symbol) {
  try {
    ...
    const [quote, companyProfile, historical] = await Promise.all([
      yfinance.quote(symbol),
      yfinance.quoteSummary(symbol, { modules: ['assetProfile', 'summaryProfile'] })
        .catch(() => null),
...
async function fetchStockSentiment(symbol) {
  try {
    // You could replace this with a real sentiment analysis API
